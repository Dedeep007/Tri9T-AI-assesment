...
from app.models.sql_models import Selection, Generation, SelectionItem, Node
...
def generate_qa_test_cases(
    ...
    # Idempotency check: return existing successful generation if force=False
    if not force:
        existing_gen = db.query(Generation)\
            .filter(Generation.selection_id == sel_id, Generation.status == "complete")\
            .order_by(Generation.created_at.desc())\
            .first()
        if existing_gen:
            print("Returning cached LLM generation.")
            return existing_gen
...
def list_selection_generations(sel_id: str, db: Session = Depends(get_db)):
    selection = db.query(Selection).filter(Selection.id == sel_id).first()
    if not selection:
        raise HTTPException(status_code=404, detail="Selection not found")
        
    return db.query(Generation)\
             .filter(Generation.selection_id == sel_id)\
             .order_by(Generation.created_at.desc())\
             .all()
